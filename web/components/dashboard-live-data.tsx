...
function RenderProgressIndicator({ attempt }: { attempt: Pick<RenderAttempt, "status" | "progress"> }) {
  const active = ["queued", "submitting_render", "rendering", "downloading_output"].includes(attempt.status);
  const measured = attempt.progress > 1;
  return (
    <div className="comfy-progress">
      <span>{renderProgressLabel(attempt)}</span>
      {active && <progress aria-label="ComfyUI rendering progress" max={100} value={measured ? attempt.progress : undefined} />}
    </div>
  );
}
...
function renderedParameterLabel(key: string): string {
  return `{{${key.toUpperCase().replaceAll(/[.-]/g, "_")}}}`;
}
...
function RenderParametersDialog({ attempt, onClose }: { attempt: RenderAttempt | null; onClose: () => void }) {
  useEffect(() => {
    if (!attempt) return;
  ...
  const preferredOrder = ["script", "video_prompt", "source_image", "audio", "duration", "audio_duration", "fps", "seed", "width", "height"];
  const entries = Object.entries(attempt.effective_values ?? {}).sort(([left], [right]) => {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="render-params-dialog" role="dialog" aria-modal="true" aria-labelledby="render-params-title"><div className="render-params-heading"><div><h2 id="render-params-title">Rendered ComfyUI parameters</h2><p>These are the effective values sent after workflow variables were expanded.</p></div><button className="toast-close" type="button" aria-label="Close rendered ComfyUI parameters" onClick={onClose}>×</button></div>{entries.length ? <div className="render-params-list">{entries.map(([key, value]) => <article className="render-param" key={key}><div><code>{renderedParameterLabel(key)}</code><small>{key}</small></div><pre>{renderedParameterValue(value)}</pre></article>)}</div> : <p className="render-params-empty">Parameters will appear after ComfyUI workflow preparation starts.</p>}</section></div>;
}
...
export function RecentJobs({ contentGenerationReady, speechGenerationReady, detailed = false, jobsOverride, topicNameOverride, contentTotal }: { contentGenerationReady: boolean; speechGenerationReady: boolean; detailed?: boolean; jobsOverride?: Job[]; topicNameOverride?: string; contentTotal?: number }) {
  ...
  const [renderParamsAttempt, setRenderParamsAttempt] = useState<RenderAttempt | null>(null);
  ...
  return (
    <div className="jobs-list">
      {jobs.map((job) => {
        const jobAttempts = attempts.data?.items.filter(
          (item) => item.job_id === job.id && !item.output_deleted_at,
        ) ?? [];
            ...
            <div className="job-card-details">
              ...
              <section className="job-results job-generation-section" aria-label={`Generation steps for ${job.topic}`}>
                ...
                <div className="job-generation-panel" role="tabpanel" hidden={generationTab !== "render"}>
                  <div className="job-render-selectors">
                    <div className="job-render-setting"><div className="job-generation-selector"><label><span>Render profile</span><select value={selectedProfileId} disabled={profileLocked || profiles.isLoading || profileMutation.isPending} onChange={(event) => setProfileSelections((current) => ({ ...current, [job.id]: event.target.value }))}><option value="" disabled>Select a render profile</option>{(profiles.data?.items ?? []).filter((item) => item.is_active).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div>{profileChanged && <button className="job-action" type="button" disabled={!selectedProfileId || profileMutation.isPending} onClick={() => profileMutation.mutate({ jobId: job.id, profileId: selectedProfileId })}>{profileMutation.isPending ? "Saving…" : "Save render profile"}</button>}</div>
                    <div className="job-render-setting"><div className="job-generation-selector"><label><span>Workflow</span><select value={selectedWorkflowId} disabled={profileLocked || workflows.isLoading || workflowMutation.isPending} onChange={(event) => setWorkflowSelections((current) => ({ ...current, [job.id]: event.target.value }))}><option value="" disabled>Select a workflow</option>{workflows.data?.items?.filter((item) => item.renderer_provider === "comfyui").map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>{selectedWorkflow && <Link className="job-detail-link" href={`/workflows#workflow-${selectedWorkflow.id}`} aria-label="Open workflow details" title="Open workflow details"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 5h5v5M19 5l-9 9M19 14v5H5V5h5" /></svg></Link>}</div>{workflowChanged && <button className="job-action" type="button" disabled={!selectedWorkflowId || workflowMutation.isPending} onClick={() => workflowMutation.mutate({ jobId: job.id, workflowTemplateId: selectedWorkflowId })}>{workflowMutation.isPending ? "Saving…" : "Save workflow"}</button>}</div>
                  </div>
                  <div className="job-render-audio"><div className="job-render-audio-copy"><strong>Render audio</strong><span>{job.audio_asset?.filename ?? "No speech file selected."}</span></div>{job.audio_asset && <audio controls preload="metadata" src={`${job.audio_asset.download_url}?inline=true`}>Your browser does not support audio playback.</audio>}<label className="job-upload-audio"><input type="file" accept="audio/*" aria-label="Upload different audio" disabled={profileLocked || audioMutation.isPending} onChange={(event) => { const file = event.target.files?.[0]; if (file) audioMutation.mutate({ jobId: job.id, file }); event.target.value = ""; }} /><span>{audioMutation.isPending && audioMutation.variables?.jobId === job.id ? "Uploading…" : "Upload different audio"}</span></label></div>
                  {profileLocked && <p className="field-hint">Render settings are locked while this content is active.</p>}{renderAction}{!attempt && <p className="field-hint">No render attempt yet.</p>}<div className="job-render-history">{jobAttempts.map((renderAttempt) => <div className={`job-render-status${renderAttempt.status === "completed" ? " completed" : ""}`} key={renderAttempt.id}><div className="job-name-with-id job-render-name"><strong>{renderAttempt.provider} · {renderAttempt.status.replaceAll("_", " ")}</strong><button className="job-id-icon" type="button" aria-label="Show rendered ComfyUI parameters" title="Rendered ComfyUI parameters" onClick={() => setRenderParamsAttempt(renderAttempt)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h10m4 0h2M4 17h2m4 0h10M14 4v6M6 14v6" /></svg></button>{renderAttempt.external_job_id && <InlineId label="ComfyUI Job ID" value={renderAttempt.external_job_id} />}</div><RenderProgressIndicator attempt={renderAttempt} />{renderAttempt.error_message && <p className="job-error" role="alert">{renderAttempt.error_message}</p>}{renderAttempt.assets.length > 0 ? <div className="job-output-list">{renderAttempt.assets.map((asset) => <div className="job-video-output" key={asset.id}><span>{asset.filename}</span><div className="job-video-actions"><button className="job-media-icon" type="button" aria-label={`Preview ${asset.filename}`} title="Preview video" onClick={() => setPreviewVideoIds((current) => ({ ...current, [asset.id]: !current[asset.id] }))}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5v14l11-7Z" /></svg></button><a className="job-media-icon" href={asset.download_url} download={asset.filename} aria-label={`Download ${asset.filename}`} title="Download video"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3v12m0 0 5-5m-5 5-5-5M5 21h14" /></svg></a><button className="job-media-icon danger" type="button" aria-label={`Delete ${asset.filename}`} title={profileLocked ? "Video deletion is locked while a rerender is active" : "Delete video"} disabled={profileLocked} onClick={() => setPendingVideoDelete(asset)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M10 11v6m4-6v6M9 7l1-2h4l1 2m-9 0 1 14h8l1-14" /></svg></button></div>{previewVideoIds[asset.id] && <video controls preload="metadata" src={`${asset.download_url}?inline=true`}>Your browser does not support video playback.</video>}</div>)}</div> : <p className="field-hint">No output files yet.</p>}</div>)}</div>
                </div>
              </section>
            </div>
          </details>;
      ...
      <RenderParametersDialog attempt={renderParamsAttempt} onClose={() => setRenderParamsAttempt(null)} />
