...
import type { DashboardSummary, Job, MediaAsset, RenderAttempt, RenderedWorkflowControl, TopicSummary } from "@/lib/api";
...
function RenderParametersDialog({ controls, onClose }: { controls: RenderedWorkflowControl[] | null; onClose: () => void }) {
  useEffect(() => {
    if (!controls) return;
...
export function RecentJobs({ contentGenerationReady, speechGenerationReady, detailed = false, jobsOverride, topicNameOverride, contentTotal }: { contentGenerationReady: boolean; speechGenerationReady: boolean; detailed?: boolean; jobsOverride?: Job[]; topicNameOverride?: string; contentTotal?: number }) {
  ...
  const [renderSettingsPreview, setRenderSettingsPreview] = useState<RenderedWorkflowControl[] | null>(null);
  ...
  return (
    <div className="jobs-list">
      ...
      <RenderParametersDialog controls={renderSettingsPreview} onClose={() => setRenderSettingsPreview(null)} />
