...
function RenderParametersDialog({ controls, onClose }: { controls: RenderedWorkflowControl[] | null; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="render-params-dialog" role="dialog" aria-modal="true" aria-labelledby="render-params-title"><div className="render-params-heading"><div><h2 id="render-params-title">Next LTX 2.3 render settings</h2><p>Current ComfyUI control values with workflow variables expanded for the next render.</p></div><button className="toast-close" type="button" aria-label="Close next render settings" onClick={onClose}>×</button></div>{controls.length ? <div className="render-params-list">{controls.map((control) => <article className="render-param" key={`${control.node_id}-${control.input_name}`}><div><strong>{control.label}</strong><small>{control.node_id}.{control.input_name}</small></div><pre>{renderedParameterValue(control.value)}</pre></article>)}</div> : <p className="render-params-empty">No supported LTX controls were found in the selected workflow.</p>}</section></div>;
}
