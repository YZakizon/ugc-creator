...
export function TopicHistory({ contentGenerationReady, speechGenerationReady }: { contentGenerationReady: boolean; speechGenerationReady: boolean }) {
  ...
  const topics = useQuery({ queryKey: ["topics"], queryFn: getTopics, refetchInterval: 5000 });
      ...
      {jobs.map((job) => {
        ...
        const speechAction = <>
          {(job.status === "content_ready" || job.status === "tts_ready" || job.status === "ready_to_render" || job.status === "completed" || failedSpeech) && (
            ...
            >
              {speechMutation.isPending && speechMutation.variables === job.id ? "Queuing…" : speechGenerationReady ? failedSpeech ? "Retry speech" : job.audio_asset ? "Generate new speech" : "Generate speech" : "ElevenLabs setup required"}
            </button>
          )}
        </>;
        const renderAction = <>
            ...
            <div className="job-card-details">
              ...
              <section className="job-results job-generation-section" aria-label={`Generation steps for ${job.topic}`}>
                ...
                <div className="job-generation-panel" role="tabpanel" hidden={generationTab !== "speech"}>
                  ...
                  <p className="field-hint">Create ElevenLabs speech from the generated script. Generate again to keep another version.</p>{speechAction}<div className="job-audio-history">{(job.audio_assets?.length ? job.audio_assets : job.audio_asset ? [job.audio_asset] : []).map((asset, index) => <div className="job-audio-result" key={asset.id}><span>{asset.filename}</span><audio controls preload="none" src={`${asset.download_url}?inline=true`}>Your browser does not support audio playback.</audio><a className="voice-preview-download" href={asset.download_url} download={asset.filename} aria-label={index === 0 ? "Download generated speech" : `Download ${asset.filename}`} title="Download audio"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3v12m0 0 5-5m-5 5-5-5M5 21h14" /></svg></a></div>)}</div>
                </div>
