...
export function previewLtxRenderControls(workflow: WorkflowTemplate, job: Job): RenderedWorkflowControl[] {
  ...
  const seed = first(items, (field) => /randomnoise/i.test(field.classType) && /noise.?seed/i.test(field.input_name));
  const controls: Array<[string, Field | undefined]> = [
    ...
    ["Seed", seed ? { ...seed, value: "Automatic — generated when rendering" } : undefined],
